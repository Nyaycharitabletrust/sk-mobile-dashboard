...
function login_(request) {
  ...
  const store = getValue_(user, ['Store', 'STORE']);
  const session = {
    username,
...
function getDashboard_(request) {
  ...
  const salesRows = session.access === 'admin'
    ? allRows
    : allRows.filter((row) => String(getValue_(row, ['Store', 'STORE']) || '').trim().toLowerCase() === String(session.storeId || '').trim().toLowerCase());
...
function toDashboardRow_(row) {
  return {
    storeName: String(getValue_(row, ['Store', 'STORE']) || ''),
